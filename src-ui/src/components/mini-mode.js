...
import { getProgressPercent } from '../mock-data.js';
...
function coverSvg(cls) {
  ...
  return `<svg viewBox="0 0 32 32"><rect width="32" height="32" fill="${fill}"/></svg>`;
}
...
export function render() {
  ...
  <div class="mini-mode${openCls}" id="miniMode" role="region" aria-label="迷你播放器" data-tauri-drag-region>
    <div class="mini-row mini-row-main">
      <button class="mini-cover ${song.coverClass}" data-action="restore" aria-label="打开播放视图">
        ${song.coverUrl ? `<img src="${song.coverUrl}" alt="">` : coverSvg(song.coverClass)}
      </button>
      <div class="mini-info" data-tauri-drag-region>
        <div class="title">${song.title}</div>
        <div class="artist">${song.artist}</div>
      </div>
      <button class="mini-icon-btn" aria-label="返回主窗口" data-action="restore-main"><i data-lucide="maximize-2"></i></button>
      <button class="mini-icon-btn mini-close-btn" aria-label="关闭迷你模式" data-action="close"><i data-lucide="x"></i></button>
    </div>
    <div class="mini-row mini-row-controls">
      <button class="mini-ctrl" aria-label="上一首" data-action="prev"><i data-lucide="skip-back"></i></button>
      <button class="mini-ctrl mini-play-btn" aria-label="${playLabel}" data-action="toggle-play"><i data-lucide="${playIcon}"></i></button>
      <button class="mini-ctrl" aria-label="下一首" data-action="next"><i data-lucide="skip-forward"></i></button>
      <span class="mini-spacer"></span>
      <button class="mini-ctrl${isLiked ? ' liked' : ''}" aria-label="${isLiked ? '已收藏' : '收藏'}" data-action="like"><i data-lucide="heart"></i></button>
      <button class="mini-ctrl" aria-label="歌词" data-action="lyrics"><i data-lucide="text"></i></button>
    </div>
...
export function bind(root) {
  ...
  el.querySelector('[data-action="restore"]')?.addEventListener('click', () => {
    if (getState().playing.song) setState({ mini: false, expanded: true, lyrics: false });
  });
  el.querySelector('[data-action="restore-main"]')?.addEventListener('click', () => {
    setState({ mini: false });
  });
  ...
  el.querySelector('[data-action="toggle-play"]')?.addEventListener('click', togglePlay);
  ...
  el.querySelector('[data-action="like"]')?.addEventListener('click', toggleLike);
